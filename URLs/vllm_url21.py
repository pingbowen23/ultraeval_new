...
parser.add_argument("--use_slide", default=None,action="store_true",help="linear")
...
def load_peft_model():
    ...
    pretrained_model = AutoModelForCausalLM.from_pretrained(
    "/data/public/opensource_models/meta-llama/Llama-2-13b-hf"
        )
    # lora_model_name_or_path = "/home/wanghanqing/projects/exp/mAlign_exp/lang_LoRAs/peft_ver/multitask_epoch2/lora"
    lora_model_name_or_path = "/home/wanghanqing/projects/exp/mAlign_exp/lang_LoRAs/peft_ver/13b/math/lora"

    #### 初始化PeftModel, 并且load第一个adapter
    lora_model = PeftModel.from_pretrained(pretrained_model, model_id = lora_model_name_or_path, adapter_name = "multi")
    ...
    lora_model.base_model.set_adapter(["multi"])
    merged_model = lora_model.base_model.merge_and_unload(adapter_names = ["multi"])
...
def load_gate_model():
    ...
    # def load_fusion_gate(model=None,gate_path="/data/public/multilingual/whq/model/gate.pt"):
    #     gate_weight_dict = torch.load(gate_path)
    #     gate_weight_dict = list(gate_weight_dict.values()) # 改顺序
        
    #     print(f"gate_weight_dict: {gate_weight_dict}")
    ...
    for i in range(32):
        ...
        pretrained_model.model.layers[i].use_slide = args.use_slide    
...
@app.route("/vllm-url21-infer", methods=["POST"])
def main():
