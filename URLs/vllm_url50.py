...
def load_model():
    ...
    def load_share_fusion_gate(model=None,gate_path=args.gate_path):
        ...
        target_dict['base_model.model.model.weight_bias'] = gate_weight_dict['model.weight_bias']
        target_dict['base_model.model.model.lora_fusion_gate.weight'] = gate_weight_dict['model.lora_fusion_gate.weight']
        
        model.load_state_dict(target_dict,strict=False)
        print("target_dict['base_model.model.model.weight_bias']: ",target_dict['base_model.model.model.weight_bias'])
    ...
    # with open('share_result/share_model_parameters.txt', 'w') as file:
    #     for name, param in lora_model.named_parameters():
    #         if "lora_fusion_gate" in name or "weight_bias" in name:
    #             print(name, file=file)
    with open('share_result/not_share_model_parameters.txt', 'w') as file:
        for name, param in lora_model.named_parameters():
            if "lora_fusion_gate" in name or "weight_bias" in name:
                print(name, file=file)
...
def load_code_model():
    ...
    lora_model.base_model.add_weighted_adapter(adapters = ['math'],weights = [0.5],adapter_name = "math2",combination_type='linear')
...
def load_zh_model(pretrained_model): # check 一下class
    ...
    lora_model.base_model.add_weighted_adapter(adapters = ['zh'],weights = [0.5],adapter_name = "zh2",combination_type='linear')
...
@app.route("/vllm-url50-infer", methods=["POST"])
def main():
