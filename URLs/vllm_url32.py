...
parser.add_argument("--gpuid", type=str, default="7", help="GPUid to be deployed")
parser.add_argument("--port", type=int, default=5032, help="the port")
...
tokenizer = AutoTokenizer.from_pretrained("/data/public/opensource_models/meta-llama/Llama-2-7b-hf")
...
def load_base_model(args):
    ...
    #     ).to(device)
    pretrained_model = AutoModelForCausalLM.from_pretrained(model_path).to(device)
    # pretrained_model = AutoModelForCausalLM.from_pretrained(
    # "/home/wanghanqing/projects/exp/mAlign_exp/lang_LoRAs/peft_ver/code"
    #     ).to(device)
...
@app.route("/vllm-url32-infer", methods=["POST"])
def main():
