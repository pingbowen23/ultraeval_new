...
parser.add_argument("--use_gate",action="store_true",help="linear")
...
def load_peft_model():
    ...
    for _ in range(32):
        pretrained_model.model.layers.temperature = args.temperature
...
def load_math_model():
    ...
    lora_model = PeftModel.from_pretrained(pretrained_model,model_id = "/home/wanghanqing/projects/exp/mAlign_exp/lang_LoRAs/peft_ver/math_epoch3/lora",adapter_name = "math")
    
    lora_model.base_model.add_weighted_adapter(adapters = ['math'],weights = [1.1],adapter_name = "math2",combination_type='linear')
...
def load_zh_model(pretrained_model): 
...
@app.route("/vllm-url1-infer", methods=["POST"])
def main():
