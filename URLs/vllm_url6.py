...
def load_peft_model():
    ...
    lora_model.base_model.add_weighted_adapter(adapters = ['fr','math'],weights = [0.0,1.0],adapter_name = "fr-math",combination_type='linear')
    
    lora_model.base_model.set_adapter(["fr-math"])
    merged_model = lora_model.base_model.merge_and_unload(adapter_names = ["fr-math"])
...
@app.route("/vllm-url6-infer", methods=["POST"])
def main():
