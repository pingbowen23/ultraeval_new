...
def Generate(prompts,model):
    ...
    if not hasattr(Generate, "seq_num"):
        Generate.seq_num = 0  # 初始化静态变量
    Generate.seq_num += 1  # 使用静态变量
    print("这是第 {} 次调用此函数。".format(Generate.seq_num))
    ...
    import shutil
    # 定义当前文件夹名称和新文件夹名称
    current_folder_name = "/data/public/multilingual/whq/UltraEval/weight_analysis/es/sample_tmp"
    new_folder_name = f"/data/public/multilingual/whq/UltraEval/weight_analysis/es/sample_{Generate.seq_num}"
    try:
        if os.path.exists(new_folder_name):
            shutil.rmtree(new_folder_name)
            print("目标文件夹 '{}' 已存在，将其删除。".format(new_folder_name))
        
        os.rename(current_folder_name, new_folder_name)
        print("文件夹名称已从 '{}' 更改为 '{}'。".format(current_folder_name, new_folder_name))
        os.mkdir(current_folder_name)
        print("重新创建文件夹 '{}'。".format(current_folder_name))
        

        ###顺便让每个layer的seq_num归0
        num_layers = model.base_model.model.model.num_hidden_layers
        for i in range(num_layers):
            model.base_model.model.model.layers[i].seq_num = 0
        


    except OSError as error:
        print("更改文件夹名称时出错: {}".format(error))
